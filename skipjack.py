# https://csrc.nist.gov/csrc/media/projects/cryptographic-algorithm-validation-program/documents/skipjack/skipjack.pdf

import sys

class SkipJack:
	def __init__(self):
		# F is an 8-bit S-box
		self.F = []
		self.defineF()
		# w1, w2, w3, w4 are 16-bit characters
		self.w1 = 0
		self.w2 = 0
		self.w3 = 0
		self.w4 = 0


	# plaintext is a 64-bit hexadecimal characters
	# key is a list of 10-bytes(80 bits)
	def encrypt(self, plaintext, key):
		self.splitWord(plaintext)

		for round in range (1, 33):
			if (1 <= round <= 8) or (17 <= round <= 24):
				self.A(round, key)
				self.printStatus(round)
			if (9 <= round <= 16) or (25 <= round <= 32):
				self.B(round, key)
				self.printStatus(round)

		return self.appendWords()


	# ciphertext is a 64-bit encrypted characters
	# key is a list of 10-bytes
	def decrypt(self, ciphertext, key):
		self.splitWord(ciphertext)

		for round in reversed(range(1, 33)):
			if (25 <= round <= 32) or (9 <= round <= 16):
				self.Binv(round, key)
				
			if (17 <= round <= 24) or (1 <= round <= 8):
				self.Ainv(round, key)
				
#here i can also use print status to print all the steps of decryption
		return self.appendWords()


	def A(self, round, key):
		c1 = self.w1
		c2 = self.w2
		c3 = self.w3
		self.w1 = self.G(round, key, c1) ^ self.w4 ^ round
		self.w2 = self.G(round, key, c1)
		self.w3 = c2
		self.w4 = c3


	def Ainv(self, round, key):
		c1 = self.w1
		c2 = self.w2
		self.w1 = self.Ginv(round, key, c2)
		self.w2 = self.w3
		self.w3 = self.w4
		self.w4 = c1 ^ c2 ^ round


	def B(self, round, key):
		c1 = self.w1
		c2 = self.w2
		c3 = self.w3
		self.w1 = self.w4
		self.w2 = self.G(round, key, c1)
		self.w3 = c1 ^ c2 ^ round
		self.w4 = c3


	def Binv(self, round, key):
		c1 = self.w1
		self.w1 = self.Ginv(round, key, self.w2)
		self.w2 = self.Ginv(round, key, self.w2) ^ self.w3 ^ round
		self.w3 = self.w4
		self.w4 = c1


	# w is a 16-bit integer
	def G(self, round, key, w):
		g = [0] * 6
		g[0] = (w >> 8 ) & 0xff
		g[1] = w & 0xff
		j = (4 * (round - 1)) % 10

		for i in range(2, 6): # gives i = 2,3,4,5
			g[i] = self.F[g[i - 1] ^ key[j]] ^ g[i - 2]
			j = (j + 1) % 10

		return (g[4] << 8) | g[5] 


	# w is a 16-bit integer
	def Ginv(self, round, key, w):
		g = [0] * 6
		g[4] = (w >> 8) & 0xff
		g[5] = w & 0xff
		j = (4 * (round - 1) + 3) % 10

		for i in reversed(range(4)): # gives i=3,2,1,0
			g[i] = self.F[g[i + 1] ^ key[j]] ^ g[i + 2]
			j = (j - 1) % 10

		return (g[0] << 8) | g[1]


	# append the four 16-bit words w1,w2,w3,w4 to return a 64-bit word
	def appendWords(self):
		x1 = self.w1 << 3 * 16
		x2 = self.w2 << 2 * 16
		x3 = self.w3 << 1 * 16
		x4 = self.w4
		return x1 | x2 | x3 | x4


	# w is a 64-bit word. This function splits w into 
	# four 16-bit words which are stored in w1, w2, w3, w4
	def splitWord(self, w):
		self.w1 = (w >> (16 * 3)) & 0xffff
		self.w2 = (w >> (16 * 2)) & 0xffff
		self.w3 = (w >> (16 * 1)) & 0xffff
		self.w4 = w & 0xffff


	# print the round number and the current values of w1,w2,w3,w4
	def printStatus(self, round):
		w = self.appendWords()
		print("round=" + str(round) + "  " + hex(w))


	def defineF(self):
		self.F = []




# main implementation
print('Enter a 64-bit or 16 digits hexadecimal to be encrypted : \n')
plaintxt = input()
if len(plaintxt) == 16:
	print('\ngreat!!! Entered hexadecimal length is valid (i.e., 64-bits) ')
else:
	print('\nTry again entering correct length of hexadecimal value')
	exit()
PT = int(plaintxt,16)

print('\nEnter a 80-bit or 10 blocks of integer key values seperated by spaces...\nExample : 06 85 78 00 64 89 27 39 18 10 ')
KEY = list(map(int,input("\nEnter the key values : ").strip().split()))

if len(KEY) > 10 :
	print("\nYou've entered more than 80-bits of key values..! \nTry again with 80-bit length key...")
	exit()
elif len(KEY) < 10 :	
	print("\nYou've entered less than 80-bits of key values..! \nTry again with 80-bit length key...")
	exit()
else:
	sj = SkipJack()
	CT = sj.encrypt(PT, KEY)
	DT = sj.decrypt(CT, KEY)
	print("\nPlain text you have entered : " + hex(PT))
	print("\nCipher text generated after 32 rounds : " + hex(CT))
	print("\nDecrypted text : " + hex(DT)) # should match the plain text

